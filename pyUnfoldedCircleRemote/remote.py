...
class Remote:
    ...
    def name(self):
        """Name of the remote."""
        return self._name or f"Unfolded Circle {self.name_from_model_id(self._model_number)}"
    ...
    async def create_api_key_revoke_if_exists(self, api_key_name=AUTH_APIKEY_NAME) -> str:
    ...
    async def start_driver_by_id(self, integration_id) -> list[dict[str, any]]:
        ...
        async with (
            self.client() as session,
            session.put(self.url(f"intg/drivers/{integration_id}?cmd=START")) as response,
        ):
            await self.raise_on_error(response)
            return await response.json()
    ...
    async def remove_remote_entities(self, entity_ids: list[str]) -> bool:
        ...
        async with (
            self.client() as session,
            session.delete(self.url("entities"), json={"entity_ids": entity_ids}) as response,
        ):
            await self.raise_on_error(response)
            return True
    ...
    async def get_all_entities_in_use(self, integration_id_filter: str = "") -> list[dict]:
    ...
    async def get_remote_charger_information(self) -> json:
        ...
        async with (
            ...
            self._is_wireless_charging_enabled = information.get("wireless_charging_enabled", False)
            return information
    ...
    async def set_standby_inhibitor(
        ...
        async with (
            self.client() as session,
            session.post(self.url("system/power/standby_inhibitors"), json=body) as response,
        ):
            await self.raise_on_error(response)
            return True
    ...
    async def remove_standby_inhibitor(self, inhibitor_id: str) -> bool:
        ...
        async with (
            self.client() as session,
            session.delete(self.url(f"system/power/standby_inhibitors/{inhibitor_id}")) as response,
        ):
            await self.raise_on_error(response)
            return True
    ...
    async def patch_remote_button_settings(
        ...
        if static_color is not None and "RGB_COLOR" in self._button_features and static_color != {}:
    ...
    async def get_remote_network_settings(self) -> str:
        ...
        async with (
            ...
            try:
                if self._model_number.upper() == "UCR3" and Version(self._sw_version) < Version(
                    "2.7.0"
                ):
    ...
    async def get_localization_settings(self) -> str:
        ...
        async with (
            ...
            self._localization_info.language_code = settings.get("language_code", "en_UK")
            self._localization_info.time_zone = settings.get("time_zone", "UTC")
            self._localization_info.time_format_24h = settings.get("time_format_24h", True)
            self._localization_info.measurement_unit = settings.get("measurement_unit", "UK")
            return settings
    ...
    async def get_physical_button_mapping(self, activity_id, button_id, hold) -> str:
        ...
        async with (
            self.client() as session,
            session.get(self.url(f"activities/{activity_id}/buttons/{button_id}")) as response,
        ):
            await self.raise_on_error(response)
            response = await response.json()
        ...
        if kwargs.get("code") is not None and kwargs.get("format") is not None:
            ...
            if not codeset_id != "" and codeset != "":
                ...
                for manufacturer in manufacturers:
                    codesets = await self.get_ir_manufacturer_codesets(manufacturer.get("id"))
                    for cs in codesets:
    ...
    async def send_ir_command_by_emitter(
        ...
        async with (
            self.client() as session,
            session.put(self.url("ir/emitters/" + emitter_id + "/send"), json=body) as response,
        ):
            await self.raise_on_error(response)
            return response.status == 200
    ...
    async def get_ir_manufacturer_codesets(self, manufacturer_id: str) -> dict[str, str]:
        if self._wake_if_asleep and self._wake_on_lan:
            if not await self.wake():
                raise RemoteIsSleeping
    ...
    def update_from_message(self, message: any) -> None:
        ...
        try:
            ...
            if data["msg"] == "ir_learning":
                ...
                self.get_dock_by_id(data["msg_data"]["device_id"])._learned_code = data["msg_data"][
                    "code"
                ]
                return
            ...
            if data["msg"] == "configuration_change":
                ...
                if state.get("button") is not None:
                    ...
                    if "RGB_COLOR" in self.button_features:
                        self._button_static_color = state.get("button").get("static_color")
                ...
                if state.get("network") is not None:
                    self._wake_on_lan = state.get("network").get("wake_on_wlan").get("enabled")
...
class UCMediaPlayerEntity:
    ...
    def update_attributes(self, attributes: any) -> dict[str, any]:
        ...
        if attributes.get("media_position_updated_at", None):
            self._media_position_updated_at = attributes.get("media_position_updated_at", None)
            attributes_changed["media_position_updated_at"] = self._media_position_updated_at
