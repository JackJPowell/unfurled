...
class Dock:
    ...
    async def delete_custom_codeset(self, codeset_device_id: str) -> dict:
        ...
        async with (
            self.client() as session,
            session.delete(self.url(f"ir/codes/custom/{codeset_device_id}")) as response,
        ):
            await self.raise_on_error(response)
    ...
    async def send_command(self, command: DockCommand, command_value: str = None) -> str:
        ...
        async with (
            self.client() as session,
            session.post(self.url(f"docks/devices/{self.id}/command"), json=payload) as response,
        ):
            await self.raise_on_error(response)
            return await response.json()
